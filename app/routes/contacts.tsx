...
export async function action({ request, context }: ActionFunctionArgs) {
  ...
  if (intent === "create") {
    try {
      ...
      if (!name || !phone) {
        return json({ error: "Name and phone are required", success: false }, { status: 400 });
      }

      if (!/^\d+$/.test(phone)) {
        return json({ error: "Phone must contain only digits", success: false }, { status: 400 });
      }
      ...
      if (error.code === "23505") {
        return json({ error: "Contact with this phone number already exists", success: false }, { status: 409 });
      }
      return json({ error: "Failed to create contact", success: false }, { status: 500 });
    }
  }
  ...
  if (intent === "update") {
    try {
      ...
      if (isNaN(id)) {
        return json({ error: "Invalid contact ID", success: false }, { status: 400 });
      }

      if (!name && !phone) {
        return json({ error: "At least one field (name or phone) must be provided", success: false }, { status: 400 });
      }
      ...
      if (!contact) {
        return json({ error: "Contact not found", success: false }, { status: 404 });
      }

      return json({ success: true, message: "Contact updated successfully!" });
    } catch (error: any) {
      console.error("Error updating contact:", error);
      if (error.code === "23505") {
        return json({ error: "Contact with this phone number already exists", success: false }, { status: 409 });
      }
      return json({ error: "Failed to update contact", success: false }, { status: 500 });
    }
  }
  ...
  if (intent === "delete") {
    try {
      ...
      return json({ error: "Failed to delete contact", success: false }, { status: 500 });
    }
  }
