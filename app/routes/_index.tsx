...
export async function loader({request, context}: LoaderFunctionArgs) {
    try {
        ...
        const status: StatusData = {
            status: services.whatsapp.getConnectionStatus(),
        ...
        return json(
            {status: {status: "error", me: null, scheduled: []}, contacts: []},
            {status: 500}
        );
    }
}
...
export async function action({request, context}: ActionFunctionArgs) {
    ...
    if (intent === "schedule") {
        try {
            ...
            if (!to || !text) {
                return json({error: "Phone number and message are required", success: false}, {status: 400});
            }
            ...
            if (mode === "once") {
                ...
                if (!timestampUTC) {
                    return json({error: "Timestamp is required for one-time messages", success: false}, {status: 400});
                }
                const timestamp = parseInt(timestampUTC, 10);
                if (isNaN(timestamp)) {
                    return json({error: "Invalid timestamp", success: false}, {status: 400});
                }
            ...
            return json({error: error.message || "Failed to schedule message", success: false}, {status: 400});
        }
    }
    ...
    if (intent === "cancel") {
        try {
            ...
            if (!jobId) {
                return json({error: "Job ID is required", success: false}, {status: 400});
            }

            const success = services.scheduler.cancelScheduledJob(jobId);
            if (!success) {
                return json({error: "Job not found", success: false}, {status: 404});
            }

            return json({success: true});
        } catch (error) {
            console.error("Error canceling job:", error);
            return json({error: "Failed to cancel job", success: false}, {status: 500});
        }
    }
    ...
    return (
        <div>
            ...
            <div className="card">
                ...
                {status.me && <div className="small">Logged in as: {status.me.id}</div>}
            </div>
            ...
            <fetcher.Form
                ...
                onSubmit={(e) => {
                    ...
                    if (mode === "once") {
                        const datetimeInput = e.currentTarget.querySelector('input[name="timestamp"]') as HTMLInputElement;
            ...
            >
                ...
                <label>
                    Message
                    <textarea name="text" required placeholder="Type your message here"/>
                </label>
