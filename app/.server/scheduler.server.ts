...
export function scheduleCron(to: string, text: string, cron: string): string {
   ...
   console.log(
      `✅ CRON job scheduled with ID: ${id}, Map size: ${scheduled.size}`,
   );
   return id;
}
...
export function scheduleOnce(
   ...
   console.log(
      `   Scheduled time: ${scheduledTime.toISOString()} (${timestamp})`,
   );
   console.log(
      `   Delay: ${delay}ms (${(delay / 1000 / 60).toFixed(2)} minutes)`,
   );
   ...
   console.log(
      `✅ One-time job scheduled with ID: ${id}, Map size: ${scheduled.size}`,
   );
   return id;
}
